...
def quantize(inputs,
    ...
    if symmetric:
        ...
        zero_point = (input_min / scale).round(
        ) * scale  # make sure the zero point is mapped to an integer value

    scale[scale.abs() == 0.] = 1.

    if symmetric:
        input_flat = (input_flat * scale + noise).round().clamp(
            -q_range // 2,
            q_range // 2 - 1)
        input_flat_scaled = input_flat / (scale if do_scale else 1.0)
    else:
        input_flat = ((input_flat - zero_point) / scale + noise).round().clamp(
            0,
...
for _ in range(1):
    ...
    #    #print(w_q[i].norm())
    print(((weight_q.float() - w_q.float()).abs() / (weight_q.abs() + 1e-6)).sum() /
          weight_q.numel())
    print(((act_q.float() - a_q.narrow(0,
                                       0,
                                       aa.size(0)).float()).abs() /
           (act_q.abs() + 1e-6)).sum() / act_q.numel())
    ...
    pt_result = (torch.matmul(act_q,
                              weight_q).reshape(aa.size(0),
                                                -1) * act_scale.unsqueeze(1)).half()
    #pt_result[pt_result.size(0)//2:, :] *= 2

    #pt_result[:, pt_result.size(1)//2:] *= 2
    print(pt_result)
    print(pt_result.shape)
    print(((pt_result.float() - ds_result.float()).abs() /
           (1e-6 + pt_result.float().abs())).sum() / ds_result.numel())
